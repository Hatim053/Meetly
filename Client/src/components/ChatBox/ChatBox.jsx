...
function ChatBox({}) {
   const [chatList , setChatList] = useState([]) // {senderId , senderName , message}
   ...
   function sendMessage() {
    ...
    if(! message) return
    socket.emit('sendMessage' , {senderId : loggedInUser?._id , senderName : loggedInUser?.name , roomId , message})
    ...
    <div className={styles["chat-body"]} id="chat-body">
     {chatList.map((chat) => (
            <div className={loggedInUser?._id == chat.senderId ? `${styles["message"]} ${styles["own"]}`:`${styles["message"]} ${styles["other"]}`}>
